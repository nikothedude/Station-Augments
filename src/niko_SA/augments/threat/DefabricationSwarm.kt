...
class DefabricationSwarm: ThreatAugment() {

    companion object {
        const val ACCEPTABLE_RANGE = 4500f * 2f
    }
    ...
    override fun getBasicDescription(tooltip: TooltipMakerAPI, expanded: Boolean, panel: CustomPanelAPI?) {
        ...
        tooltip.addPara(
            "Coordinates nearby fragments into carnivorous swarms that tear apart ships and return their material to the host swarm.",
            0f
        )
        tooltip.addPara(
        ...
        tooltip.addPara(
            "Uses %s fragments per volley. It will not fire if a target is phased or has shields mostly raised.",
            5f,
            Misc.getHighlightColor(),
            "six"
        )
    ...
    override fun modifyAugmentMenu(tooltip: TooltipMakerAPI, panel: CustomPanelAPI?, buttonPanel: CustomPanelAPI?, delegate: AugmentMenuDialogueDelegate) {
        ...
        val slider = LunaUITextFieldWithSlider<Int>(level.frontend, Level.ONE.frontend.toFloat(), 3f, 100f, 50f, "SA_defabFragSlider", "SA_defabragSlider", panel, tooltip)
    ...
    override fun applyInCombat(station: ShipAPI) {
        val plugin = DefabFragmentMimic(station)
        plugin.acceptableRange = ACCEPTABLE_RANGE
        plugin.modifyHostShip = { it.mutableStats.missileWeaponRangeBonus.modifyMult("SA_defFragRange", 1.5f) }
        plugin.unModifyHostShip = { it.mutableStats.missileWeaponRangeBonus.unmodify("SA_defFragRange") }
        plugin.bonusCanFireCheckTarget = end@{ ship, target ->
            if (ship.shield != null && ship.shield.activeArc > 45f && SA_mathUtils.prob(95)) {
                if (ship.fluxTracker.fluxLevel < 0.9f) return@end false
            }
    ...
    class DefabFragmentMimic(station: ShipAPI): FragWeaponMimic("devouring_swarm", "devouring_swarm_fire", station, 6f) {
        override fun getWeightForShip(ship: ShipAPI): Float {
            val base = super.getWeightForShip(ship)
            var total = base

            if (ship.shield == null) total *= 10f
